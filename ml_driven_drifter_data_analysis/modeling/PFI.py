...
for model in ML_model_settings:
    output_path = FIGURES_DIR / 'PFI'/ f'pfi_{model}_vertical'
    ...
    if 'SVR' not in model:
        ...
        plot_pfi_shadow_vertical(upfi, vpfi, ustats, vstats, labels, output_path, residual=('residual' in model), bar_width=0.6, title='Random Forest')
    else:
        plot_pfi_shadow_vertical(upfi, vpfi, ustats, vstats, labels, output_path, residual=('residual' in model), bar_width=0.6, ylabel=False, title='Support Vector Regression')
# %%
...
for model in ML_model_settings:
    ...
    plot_pfi_shadow(upfi, vpfi, ustats, vstats, labels, output_path, residual=('residual' in model), bar_width=0.6, )
# %%
labels=pd.read_csv(f'{PROJ_ROOT}/references/variables_labels.csv', delimiter=';', index_col=0)
